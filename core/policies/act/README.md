...
```bash
python core/policies/act/train.py   --env-id grasp_part   --object-name bottle   --part-name cap   --demo-path demos/output.state+rgb.pd_ee_delta_pos.physx_cpu.h5   --control-mode pd_ee_delta_pos   --num_demos 40   --total_iters 100000   --exp-name act-grasp-cabinet-handle --max_episode_steps 100
```

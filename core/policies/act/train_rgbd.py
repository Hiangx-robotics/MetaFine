...
sys.path.insert(0, '/export/xuhy/EAI/git')  # FGManip root
...
class Args:
    ...
    object_name: str = "100920"
    """the name of the object"""
    part_name: str = "button"
    """the name of the part"""
    target_letter: Optional[str] = None
    """target_letter kwarg for assembling_kits_metafine_letter env (M/E/T/A/F/I/N)."""
...
def build_eval_profiles(base_env_kwargs: Dict, args: Args) -> Dict[str, Dict]:
    ...
    # Semantic perturbation: same env but with target_letter="N" (scene swap)
    if base_env_kwargs.get("target_letter", None) is not None or args.env_id == "assembling_kits_metafine_letter":
        profiles["sem_scene_n"] = {
            **base_env_kwargs,
            "target_letter": "N",
            "eval_randomize_camera": False,
            "eval_randomize_light": False,
...
if __name__ == "__main__":
    ...
    # Only pass object/part kwargs for environments that support them.
    envs_with_object_part = {
        "grasp_part",
        "stand_up",
        "toggle_switch",
        "toggle_switch_table",
    ...
    if args.env_id in envs_with_object_part:
        env_kwargs["object_name"] = args.object_name
        env_kwargs["part_name"] = args.part_name
    if args.env_id == "assembling_kits_metafine_letter" and args.target_letter is not None:
    ...
    eval_profiles = build_eval_profiles(env_kwargs, args)  # ACT_LAZY_EVAL
    eval_envs = {}
    # Only the `clean` profile stays alive across training; non-clean profiles
    # are created/closed per-eval to avoid 8x physx_cuda contexts blowing GPU mem.
    clean_video_dir = f"runs/{run_name}/videos/clean" if args.capture_video else None
    eval_envs["clean"] = make_eval_envs(
        args.env_id,
        args.num_eval_envs,
        args.sim_backend,
        eval_profiles["clean"],
        other_kwargs,
        video_dir=clean_video_dir,
        wrappers=wrappers,
    )
        ...
        if cur_iter % args.eval_freq == 0:
            ...
            for profile_name, profile_env_kwargs in eval_profiles.items():
                if profile_name == "clean":
                    profile_env = eval_envs["clean"]
                    own_env = False
                else:
                    video_dir = (f"runs/{run_name}/videos/{profile_name}"
                                 if args.capture_video else None)
                    profile_env = make_eval_envs(
                        args.env_id,
                        args.num_eval_envs,
                        args.sim_backend,
                        profile_env_kwargs,

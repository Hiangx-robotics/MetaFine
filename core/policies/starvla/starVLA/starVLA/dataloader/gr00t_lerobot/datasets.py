...
class LeRobotSingleDataset(Dataset):
        ...
        # v3.0
            ...
            for file_path in file_paths:
                ...
                for index, episode in episodes_data.iterrows():
                    ...
                    for col in episodes_data.columns:
                        if col.startswith("videos/") and col.endswith("/from_timestamp"):
                            episode_meta[col] = episode[col]
    ...
    def get_trajectory_data_lerobot_v3(self, trajectory_id: int) -> pd.DataFrame:
        ...
        if self.curr_traj_id == trajectory_id and self.curr_traj_data is not None:
            ...
            video_timestamp_key = None
            for video_key in self.lerobot_modality_meta.video.keys():
                original_key = self.lerobot_modality_meta.video[video_key].original_key
                if original_key is None:
                    original_key = video_key
                timestamp_key = f"videos/{original_key}/from_timestamp"
                if timestamp_key in self.trajectory_ids_to_metadata[trajectory_id]:
                    video_timestamp_key = timestamp_key
                    break

            if video_timestamp_key:
                from_timestamp = self.trajectory_ids_to_metadata[trajectory_id][video_timestamp_key]
    ...
    def get_episode_file_index(self, ep_index: int) -> int:
        ...
        trajectory_id = self.trajectory_ids[ep_index]

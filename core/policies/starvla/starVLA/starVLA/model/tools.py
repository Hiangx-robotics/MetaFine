...
#         assert checkpoint_pt.suffix in (".pt", ".bin", ".safetensors"), f"Unsupported checkpoint format: {checkpoint_pt.suffix}"
...
def read_mode_config(pretrained_checkpoint):
    ...
    if os.path.isfile(pretrained_checkpoint):
        ...
        # run_dir = checkpoint_pt.parents[1]
        run_dir = None
        for parent in checkpoint_pt.parents:
            if (parent / "config.yaml").exists() and (parent / "dataset_statistics.json").exists():
                run_dir = parent
                break

        if run_dir is None:
            raise FileNotFoundError(
                f"Could not find `config.yaml` and `dataset_statistics.json` in any parent of {checkpoint_pt}"
        )

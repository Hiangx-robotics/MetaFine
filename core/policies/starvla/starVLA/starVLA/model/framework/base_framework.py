...
class baseframework(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        Supports both full checkpoints (.pt/.bin) and PEFT LoRA adapters (adapter_model.safetensors).
        """
        pretrained_checkpoint = Path(pretrained_checkpoint)

        # Step 1: Load config and norm stats
        model_config, norm_stats = read_mode_config(pretrained_checkpoint)
        config = dict_to_namespace(model_config)
        config.trainer.pretrained_checkpoint = None

        # Step 2: Build base model (without LoRA)
        base_model = build_framework(cfg=config)
        base_model.norm_stats = norm_stats

        # Step 3: Determine if this is a LoRA adapter or full checkpoint
        if pretrained_checkpoint.name == "adapter_model.safetensors" or \
        pretrained_checkpoint.name == "adapter_model.bin":
            # It's a PEFT LoRA adapter → load via PeftModel
            from peft import PeftModel
            adapter_dir = pretrained_checkpoint.parent  # e.g., steps_30000/
            ...
            # It's a full model checkpoint (.pt, .bin)
            if pretrained_checkpoint.suffix.lower() == ".safetensors":
                from safetensors.torch import load_file
                state_dict = load_file(pretrained_checkpoint)
            else:

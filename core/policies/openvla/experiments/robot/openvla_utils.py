...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    # Set number of images in model input when supported by the loaded backbone.
    # Older checkpoints may expose a backbone implementation without this helper.
    if hasattr(vla.vision_backbone, "set_num_images_in_input"):
        vla.vision_backbone.set_num_images_in_input(cfg.num_images_in_input)
    elif cfg.num_images_in_input != 1:
        print(
            "Warning: loaded vision backbone does not support `set_num_images_in_input`; "
            "falling back to single-image input."
        )
...
def get_vla_action(
    ...
    with torch.inference_mode():
        ...
        if action_head is None:
            # Standard OpenVLA output: a single unnormalized action vector.
            prediction = vla.predict_action(**inputs, unnorm_key=cfg.unnorm_key, do_sample=False)
            action = prediction[0] if isinstance(prediction, tuple) else prediction

            # Normalize return type so the rollout loop can keep using an action queue.
            action = np.asarray(action, dtype=np.float32)
            if action.ndim == 1:
                return [action]
            return [np.asarray(step_action, dtype=np.float32) for step_action in action]

        # Optional compatibility path for OFT-style continuous action heads.

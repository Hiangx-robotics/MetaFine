...
Evaluates OpenVLA-OFT policy in various ManiSkill environments.
"""
...
# Assumes this file is at core/policies/openvla-oft/evaluate.py
# Root is ../../../
...
class GenerateConfig:
    ...
    seed: int = 7
    unnorm_key: Optional[str] = "fgmanip_rlds"
...
def save_rollout_video(rollout_images, idx, success, task_description, video_dir,
    ...
    mp4_path = f"{video_dir}/{DATE_TIME}--openvla_oft--episode={idx}--success={success}{obj_tag}{part_tag}--task={processed_task_description}.mp4"
...
def _run_task_graph_eval(cfg: GenerateConfig, model, processor, action_head,
    ...
    print(f"\n{'='*48}")
    print(f"openvla-oft task-graph eval: {cfg.task_graph}")
...
def eval_policy(cfg: GenerateConfig) -> None:
    print(f"Evaluating OpenVLA-OFT on {cfg.task_name}...")

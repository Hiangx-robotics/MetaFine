...
1. Auto-detects original image resolution (Fixes blurriness).
2. Auto-detects available video codec (Fixes "Encoder not found" errors).
3. Correctly writes the actual used codec to metadata (Fixes metadata mismatch).
...
class Args:
    ...
    """Output image size as WIDTHxHEIGHT. If None, uses original resolution (Recommended)."""
    
    robot_type: Optional[str] = None
    """Robot type (default: auto-detected, e.g., "panda", "ur5")"""

    max_episodes: Optional[int] = None
    """Maximum number of episodes/trajectories to convert (from the beginning)."""
...
def check_video_codec(fps: int = 30) -> str:
    """
    Checks if H.264 (avc1) is available. If not, falls back to mp4v.
    Returns the fourcc string of the available codec.
    """
    ...
    # Try avc1 first
    try:
        # We redirect stderr to suppress the noisy "Encoder not found" error from OpenCV C++
        # Note: This stderr redirection might not catch all C++ output depending on OS/setup,
        # but the logic handles the availability check regardless.
        fourcc = cv2.VideoWriter_fourcc(*"avc1")
        out = cv2.VideoWriter(temp_path, fourcc, fps, (64, 64))
        if out.isOpened():
            out.release()
            os.remove(temp_path)
            return "avc1"
    ...
    logger.warning("H.264 (avc1) encoder not found. Falling back to 'mp4v'. Video quality might be slightly lower.")
    return "mp4v"
...
def create_video_from_frames(
    ...
    if len(frames) == 0:
        logger.warning(f"Empty frames for video: {output_path}")
        return

    source_h, source_w = frames[0].shape[:2]
    if source_h == image_height and source_w == image_width:
        processed_frames = frames
    else:
        target_size = (image_width, image_height)
        processed_frames = [resize_image_with_padding(frame, target_size) for frame in frames]
    
    fourcc = cv2.VideoWriter_fourcc(*codec)
    out = cv2.VideoWriter(str(output_path), fourcc, fps, (image_width, image_height))
    
    if not out.isOpened():
        raise RuntimeError(f"Failed to create video writer for {output_path} using codec {codec}")
...
def calculate_statistics(
    ...
    # --- action stats: mean/std/max/min plus q01/q99 quantiles. The
    # quantile fields are needed by pi05's QUANTILE normalizer; older
    # converter versions used to omit them, which surfaced as a
    # `QUANTILES normalization mode requires q01 and q99 stats` error.
    # ---
    ...
    # --- observation.state stats: same q01 / q99 treatment as the action
    # block above, so downstream policies that normalise state can find
    # the same quantile fields. ---
    ...
    for camera_name, rgb_data in all_rgb_data_by_camera.items():
        if rgb_data:
            ...
            stats[f"observation.images.{camera_name}"] = {
                ...
                "q01": [[[float(np.percentile(all_pixels[:, i], 1))]] for i in range(3)],
                "q99": [[[float(np.percentile(all_pixels[:, i], 99))]] for i in range(3)],
...
def create_meta_files(
    ...
    for ep_idx, (length, df) in enumerate(zip(episode_lengths, all_dataframes)):
        ...
        # Per-episode q01 / q99 / count, matching the dataset-level fields.
        episode_meta["stats/action/q01"] = np.percentile(actions, 1, axis=0).tolist()
        episode_meta["stats/action/q99"] = np.percentile(actions, 99, axis=0).tolist()
        ...
        if state_dim and "observation.state" in df:
            ...
            episode_meta["stats/observation.state/q01"] = np.percentile(states, 1, axis=0).tolist()
            episode_meta["stats/observation.state/q99"] = np.percentile(states, 99, axis=0).tolist()
        ...
        for camera_name in rgb_cameras:
            if camera_name in all_rgb_data_by_camera and ep_idx < len(all_rgb_data_by_camera[camera_name]):
                ...
                episode_meta[f"{prefix}/q01"] = [
                    [[float(np.percentile(rgb_data[..., i], 1))]] for i in range(3)
                ]
                episode_meta[f"{prefix}/q99"] = [
                    [[float(np.percentile(rgb_data[..., i], 99))]] for i in range(3)
                ]
    ...
    for camera_name in rgb_cameras:
        features[f"observation.images.{camera_name}"] = {
            ...
            "info": {
                ...
                "video.channels": 3,
                "video.codec": video_codec,  # Uses the actual detected codec
...
def main(args: Args):
    ...
    try:
        ...
        # --- Optional: only convert first N episodes ---
        if args.max_episodes is not None:
            if args.max_episodes <= 0:
                raise ValueError("--max-episodes must be positive")
            if args.max_episodes < len(episodes):
                logger.info(f"Using only first {args.max_episodes} episodes out of {len(episodes)}")
                episodes = episodes[: args.max_episodes]
        # -------------------------------------------

        # --- Logic for Auto-Detecting Image Size ---
        first_rgb_frame = None
        for cam_name in info['rgb_cameras']:
            key = f'rgb_{cam_name}'
            if episodes and key in episodes[0]:
                first_rgb_frame = episodes[0][key][0]
                break
        
        if args.image_size is None:
            if first_rgb_frame is not None:
                image_height, image_width = first_rgb_frame.shape[:2]
                logger.info(f"Auto-detected original image size: {image_width}x{image_height}")
            else:
                image_width, image_height = 640, 480
                logger.warning("Could not detect image size from data, defaulting to 640x480")
        else:
            image_width, image_height = parse_image_size(args.image_size)
        # -------------------------------------------

        # --- Check Video Codec Once ---
        logger.info("Checking available video codecs...")
        active_codec = check_video_codec(args.fps)
        logger.info(f"Using video codec: {active_codec}")
        # ------------------------------
        ...
        for episode_idx, episode_data in enumerate(tqdm(episodes, desc="Processing episodes")):
            ...
            for camera_name in info['rgb_cameras']:
                ...
                if rgb_key in episode_data:
                    ...
                    create_video_from_frames(episode_data[rgb_key], video_path, args.fps, image_width, image_height, codec=active_codec)
        ...
        create_meta_files(
            ...
            video_codec=active_codec,  # Pass the correct codec
        ...
        logger.info(f"Video Codec: {active_codec}")
